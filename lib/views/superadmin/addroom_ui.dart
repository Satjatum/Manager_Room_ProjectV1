  ...
  Future<void> _loadAllData() async {
    ...
    try {
      ...
      if (_categories.isNotEmpty && _selectedCategoryId == null) {
        final standardCategory = _categories.firstWhere(
          (cat) => cat['category_code'] == 'standard',
          orElse: () => _categories.first,
        );
        _selectedCategoryId = standardCategory['category_id'];
      }

      if (_roomTypes.isNotEmpty && _selectedTypeId == null) {
        final singleType = _roomTypes.firstWhere(
          (type) => type['type_code'] == 'single',
          orElse: () => _roomTypes.first,
        );
        _selectedTypeId = singleType['type_id'];
        _maxOccupants = singleType['default_max_occupants'] ?? 1;
      }

      if (_statusTypes.isNotEmpty && _selectedStatusId == null) {
        final availableStatus = _statusTypes.firstWhere(
          (status) => status['status_code'] == 'available',
          orElse: () => _statusTypes.first,
        );
        _selectedStatusId = availableStatus['status_id'];
      }
    } catch (e) {
